...
def fidelity_simulation_P4_symmetric_weights(fixed_initial_edge_weights, initial_runtime_before,
                                             ...
                                             target_fidelity=0.9999999, stagnation_threshold=0.0001,
                                             stagnation_window=1000):
    """
    Optimizes the final edge weights and runtimes of a symmetric P4 graph Hamiltonian to achieve a target fidelity
    for transferring the quantum state from node 1 to node 4.

    Parameters:
    fixed_initial_edge_weights (list of float): Fixed symmetric edge weights for the Hamiltonian during the first phase.
    ...
    initial_final_edge_weights (list of float): Initial symmetric edge weights during the second phase.
    ...
    def build_hamiltonian_tf(edge_weights):
        """Construct a symmetric Hamiltonian for a P4 graph using TensorFlow."""
        h = tf.zeros((4, 4), dtype=tf.float32)

        # Enforce symmetry: w1 = w3
        w1 = edge_weights[0]
        w2 = edge_weights[1]
    ...
    final_edge_weights_tf = tf.Variable(initial_final_edge_weights[:2],
                                        dtype=tf.float32)  # Only w1 and w2 are independent
    ...
    # Fixed initial edge weights as TensorFlow constants
    initial_edge_weights_tf = tf.constant(fixed_initial_edge_weights[:2],
                                          dtype=tf.float32)  # Only w1 and w2 are independent
    ...
    for step in range(100000):  # Maximum iterations
        with tf.GradientTape() as tape:
            ...
            # Calculate the final fidelity using the symmetric final edge weights
        ...
        if step >= stagnation_window:
            ...
            if improvement < stagnation_threshold:
                ...
                # Perturb runtimes and edge weights to escape local minima
    ...
    print("Final Edge Weights (Final):",
          [final_final_edge_weights[0], final_final_edge_weights[1], final_final_edge_weights[0]])
...
# Example usage
fixed_initial_edge_weights = [1.0, 1.0, 1.0]  # Fixed symmetric initial edge weights
initial_runtime_before = 2.790979  # Initial guess for runtime before adjustment
initial_runtime_after = 2.4887805  # Initial guess for runtime after adjustment
initial_final_edge_weights = [1.2685894, 0.10852879, 1.2685894]  # Initial guess for symmetric final edge weights
fidelity_simulation_P4_symmetric_weights(fixed_initial_edge_weights, initial_runtime_before, initial_runtime_after,
