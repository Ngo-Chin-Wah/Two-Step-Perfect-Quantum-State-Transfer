...
def fidelity_simulation_P4_symmetric_weights(fixed_initial_edge_weights, initial_runtime_before,
    ...
    target_state = np.array([0, 1, 0, 0], dtype=np.complex128)
    ...
    # Initial edge weights are trainable
    ...
    pd.DataFrame(solutions).to_csv("solutions_n1 to n2_w1 -w2 w1.csv", index=False)
...
fixed_initial_edge_weights = [10.0, 3.0, 10.0]  # Initial guess for edge weights (w1, w2, w3 where w3 = w1)
initial_runtime_before = 5.0  # Initial runtime before the state transfer
initial_runtime_after = 5.0   # Initial runtime after the state transfer
