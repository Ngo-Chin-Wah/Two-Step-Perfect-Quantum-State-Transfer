...
def fidelity_simulation_P4_symmetric_weights(fixed_initial_edge_weights, initial_runtime_before,
                                             initial_runtime_after, target_fidelity=0.999,
    ...
    for step in range(500000):
        ...
        if loss_value < (1 - target_fidelity):
            ...
            while True:
                runtime_before.assign(tf.maximum(1.0 + tf.random.normal([], mean=0.0, stddev=1.0, dtype=tf.float64), 0.0))
                runtime_after.assign(tf.maximum(1.0 + tf.random.normal([], mean=0.0, stddev=1.0, dtype=tf.float64), 0.0))
        ...
        if step >= stagnation_window:
            ...
            if improvement < stagnation_threshold:
                ...
                # Perturb runtime_before with a valid value
                while True:
                    runtime_before.assign(tf.maximum(1.0 + tf.random.normal([], mean=0.0, stddev=3, dtype=tf.float64), 0.0))
                    runtime_after.assign(tf.maximum(1.0 + tf.random.normal([], mean=0.0, stddev=1.0, dtype=tf.float64), 0.0))
                    if runtime_before.numpy() <= 5.0 and runtime_after.numpy() <= 5.0:
                        break  # Exit the loop if runtime is valid

    pd.DataFrame(solutions).to_csv("solutions_n1 to n4_-1 w2 -1_ 0.999_different time.csv", index=False)
