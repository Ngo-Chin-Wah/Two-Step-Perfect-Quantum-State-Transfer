...
# Function to optimize runtimes and symmetric edge weights of a P4 graph to achieve target fidelity
def fidelity_simulation_P4_symmetric_weights(fixed_initial_edge_weights, initial_runtime_before,
    ...
    """
    Optimize runtimes and symmetric edge weights of a P4 graph Hamiltonian
    to achieve a target fidelity for quantum state transfer from node 1 to node 4.

    Both initial and final edge weights are trained, and symmetry (w1 = w3) is enforced.

    Parameters:
    fixed_initial_edge_weights (list of float): Initial guess for edge weights before switching.
    ...
    initial_final_edge_weights (list of float): Initial guess for final phase edge weights.
    target_fidelity (float, optional): Target fidelity to reach (default 1.0).
    stagnation_threshold (float, optional): Minimum fidelity improvement to detect stagnation (unused here).
    stagnation_window (int, optional): Number of iterations to monitor stagnation (unused here).
    ...
    def build_hamiltonian_tf(edge_weights):
        ...
        Parameters:
        edge_weights (list or Tensor): List of edge weights [w1, w2, w1] for edges (0-1), (1-2), (2-3).
    ...
    # Define initial and target states for P4 graph
    ...
    target_state[2] = 1.0  # Target excitation at node 3
    ...
    for step in range(200000):
        with tf.GradientTape() as tape:
            # Enforce symmetry for both initial and final edge weights
        ...
        # Enforce maximum runtime constraint
...
# Define parameters and run the optimization
fixed_initial_edge_weights = [2.0, 3.0, 2.0]  # Initial edge weights for the first Hamiltonian
initial_runtime_before = 7.0  # Initial runtime before adjustment
initial_runtime_after = 12.0  # Initial runtime after adjustment
initial_final_edge_weights = [-2.0, 0.5, -1.0]  # Initial final edge weights for second Hamiltonian
